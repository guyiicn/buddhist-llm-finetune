...
基于 Qwen2.5 的佛经知识大模型 LoRA 微调项目。从 CBETA 电子佛典提取经典原文，使用 RAG + Qwen API 生成高质量问答数据，训练专业佛学知识问答模型「善知识」。

## 项目特点

- 📚 **18 部经典原文** — 佛经十三经 + 5 部重要论典，共 71 万字
- 🤖 **12,285 条高质量训练数据** — RAG 增强 + Qwen-plus 生成，覆盖义理、修行、问答等多维度
- 🎭 **专项数据增强** — 幻觉控制、边界拒答、身份强化、唯识学专项
- ⚡ **LoRA 微调** — 支持 Jetson/GPU 训练，高效部署
- 🎯 **「善知识」佛学问答** — 专业、如法、有边界
...
| 类别 | 数量 | 说明 |
|------|------|------|
| hallucination_defense | 300 | 幻觉控制 - 训练模型说"不确定" |
| boundary | 500 | 边界拒答 - 12类非佛学问题 |
| identity | 500 | 身份强化 - 善知识 + 开经偈 |
| weishi | 200 | 唯识学专项 - 八识/三性/转识成智 |
...
│
├── v2_training/             # v2 训练数据 (10,785 条)
│   ├── buddhist_full_train.json    # 训练集
│   ├── buddhist_full_val.json      # 验证集
│   ├── train_buddhist_v2.py        # 训练脚本
│   └── start_train_v2.sh           # 启动脚本
│
├── v3_training/             # v3 专项数据 + 合并数据
│   ├── v3_training_data.json       # 1,500 条专项数据
│   ├── buddhist_v3_train.json      # 12,285 条合并训练集
│   ├── buddhist_v3_val.json        # 验证集
│   ├── gen_boundary.py             # 边界拒答数据生成
│   ├── gen_identity.py             # 身份强化数据生成
│   ├── gen_weishi.py               # 唯识学数据生成
│   ├── train_buddhist_v3.py        # v3训练脚本
│   └── README.md                   # v3数据说明
│
├── benchmark/               # 模型评测
│   ├── buddhist_benchmark.py       # 基准测试脚本
...
│
├── cleaned/                 # 清洗后的分块数据
│   └── chunks.json          # 1,569 个文本块
│
├── qa_pairs/               # 生成的 QA 数据
│   └── buddhist_qa.json     # 4,374 条
│
├── output/                 # Alpaca 格式数据
